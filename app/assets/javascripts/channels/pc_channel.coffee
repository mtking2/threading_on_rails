	...
	received: (data) ->
		...
		switch data.action
			when "item_produced"
				...
				color_patch = document.querySelector("[data-producer-id='#{data.id}'] .color-patch")
			...
			when "item_consumed"
				...
				if item
					...
					color_patch = document.querySelector("[data-consumer-id='#{data.id}'] .color-patch")
					color_patch.style.backgroundColor = "##{data.color}"
					console.log(color_patch.style.backgroundColor)
			...
			when "producer_destroyed"
				document.querySelector("[data-producer-id='#{data.id}']").remove()
				if data.destroy_resources
					document.querySelectorAll(".item[data-thread-id='#{data.id}']").forEach((item) => item.remove())
			when "destroy_resources"
				setTimeout (->
					document.querySelectorAll('.item').forEach (item) -> item.remove()
				), 250
			when "consumer_destroyed"
				document.querySelector("[data-consumer-id='#{data.id}']").remove()
			when "status_changed"
